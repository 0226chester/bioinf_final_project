...
def analyze_network_topology(data, hub_percentile=90):
    ...
    if G.number_of_nodes() > 0:
        ...
        if degree_sequence:
            ...
            # Ensure degree_dict is not empty before calculating percentile
            if degree_dict:
                 # Use np.percentile correctly; it expects an array-like input
                degrees_for_percentile = list(degree_dict.values())
...
def generate_topology_report(stats, node_idx_to_protein_id=None, filepath=None, top_n_hubs_to_show=5):
    ...
    if node_idx_to_protein_id and stats.get('hub_nodes_indices'):
        ...
        # Sort hubs by degree (descending) to show most connected if degrees are available
        # For simplicity, here we just take the first few from the list.
        # A more robust way would be to sort hub_nodes_indices by their actual degrees.
    ...
    report.extend([
        ...
        # (Interpretations remain largely the same, but are now based on more robust stats)
...
def identify_functional_modules(data, node_embeddings, method='kmeans', k_range=None, dbscan_eps=0.5, dbscan_min_samples=5):
    ...
    if method == 'kmeans':
        if k_range is None:
            # Ensure k_range is valid: upper bound should be less than num_nodes
        ...
        if num_clusters == 0 and -1 in cluster_labels: # All points are noise
            ...
            # Optionally, re-label all noise points to cluster 0 for consistent module naming
    ...
    for node_idx, module_idx in enumerate(cluster_labels):
        # Map DBSCAN noise (-1) to a specific module name if desired, or handle separately
        module_key = int(module_idx) if module_idx != -1 else "Noise" 
        modules[module_key].append(int(node_idx))
    
    # Create module assignments, ensuring consistent naming
    modules_dict = {}
    module_counter = 1
    noise_nodes = []
    ...
    if noise_nodes:
         ...
         # num_identified_modules should reflect actual clusters, not the noise "cluster"
         num_identified_modules = num_clusters 
    else:
...
def visualize_functional_modules(data, node_embeddings, save_path=None, clustering_method='kmeans', **cluster_kwargs):
    ...
    if not modules_dict or num_effective_modules == 0:
        ...
        # Fallback: visualize all nodes as one module if modules_dict is empty but nodes exist
    ...
    # Create colors for each module
    # Handle the case where num_effective_modules might be 0 or 1
    ...
    # Separate drawing for non-noise and noise nodes for clarity
    
    # Draw non-noise modules
    ...
    # Draw noise nodes if they exist
...
def analyze_prediction_patterns(model, data, device, degree_percentile_threshold=50):
    ...
    # Build graph from message-passing edges (data.edge_index) to calculate degrees
    G_msg = nx.Graph()
    ...
    for category in results:
        ...
        # Add precision for this category
        results[category]['precision'] = results[category]['true_pos'] / results[category]['count'] if results[category]['count'] > 0 else 0
        # Add recall for this category (TP / actual positives in this category)
        results[category]['recall'] = results[category]['true_pos'] / results[category]['actual_pos'] if results[category]['actual_pos'] > 0 else 0
