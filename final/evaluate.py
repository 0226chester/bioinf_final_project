...
def plot_precision_recall_curve(model, test_graphs, device, filepath=None):
    ...
    # Collect predictions
    with torch.no_grad():  # Also good practice to use no_grad for evaluation
        for data in test_graphs:
            data = data.to(device)
            if data.edge_label_index is None or data.edge_label_index.numel() == 0:
                continue
    ...
    # Concatenate results - now no need for .detach() since it's already detached
    final_preds = torch.cat(all_preds, dim=0).numpy()
    final_labels = torch.cat(all_labels, dim=0).numpy()
    ...
    # Add a baseline
    plt.plot([0, 1], [np.sum(final_labels) / len(final_labels)] * 2, 'r--', linewidth=2, 
             label='Baseline (Random Predictor)')
